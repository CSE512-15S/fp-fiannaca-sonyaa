 ...
 * @param {boolean}     [isTemporary=false] Indicates if this is a temporary edge used when initially connecting two nodes
 *
 * @constructor Calculates the points of the start and end of the edge.
 */
function FlowEdge(start, end, direction, isTemporary) {
    isTemporary === undefined ? this.temp = false : this.temp = true;
 ...
 */
FlowEdge.prototype.Update = function(hasStartOffset, hasEndOffset) {
    if(hasStartOffset === undefined) {
        hasStartOffset = true;
    }

    if(hasEndOffset === undefined) {
        hasEndOffset = true;
    }
    ...
    if(hasStartOffset === true) {
        this.startPt = {
            x: this.start.x + offset * this.dx + nodeWidth,
            y: this.start.y + offset * this.dy + nodeWidth
        };
    } else {
        this.startPt = {
    ...
    if(hasEndOffset === true) {
        this.endPt = {
            x: this.end.x - offset * this.dx + nodeWidth,
            y: this.end.y - offset * this.dy + nodeWidth
        };
    } else {
        this.endPt = {

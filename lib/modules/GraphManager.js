...
function GraphManager(flowviz) {
    ...
    var types = fv.Config.getLeafNodesTypes();
    ...
    this.AddEdge(this.nodes[0], this.nodes[2], FlowEdge.FORWARD, false);
    this.AddEdge(this.nodes[1], this.nodes[3], FlowEdge.FORWARD, false);
    this.AddEdge(this.nodes[2], this.nodes[3], FlowEdge.BACKWARD, false);
    this.AddEdge(this.nodes[3], this.nodes[2], FlowEdge.BACKWARD);
...
GraphManager.prototype.CheckNewEdge = function(start, end, dir) {
    ...
    _.forEach(this.edges, function(edge) {
        if(edge.start === start && edge.end === end) {
            console.error("No edges will be added since this edge already exists!");

            addEdge = false;
            return false;
        }
        if(edge.start === end && edge.end === start) {
            console.warn("This opposite edge exists...");

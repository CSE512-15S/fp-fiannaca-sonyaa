 ...
 * @param {object}      edges   The list of edges in the graph
 * @param {bool}        isOutgoing  If true, return outgoing types, otherwise return incoming.
 * @param {bool}        isRequired  If true, return required types, otherwise return possible. TODO
 * @return {object}    List of NodeType objects corresponding to required nodes
 */
function GetMissingTypes(node, edges, isOutgoing, isRequired) {
    var constraints = node.type.GetConstraintObject();
    var neighbors = [];
    var neighborCountByType = {};

    _.forEach(edges, function (edge) {
        if (edge.start === node && isOutgoing) {
            neighbors.push(edge.end);
            if (!(edge.end.type in neighborCountByType)) {
                neighborCountByType[edge.end.type] = 0;
            }
            neighborCountByType[edge.end.type] += 1;
        }
        if (edge.end === node && !isOutgoing) {
            neighbors.push(edge.start);
            if (!(edge.start.type in neighborCountByType)) {
                neighborCountByType[edge.start.type] = 0;
            }
            neighborCountByType[edge.start.type] += 1;
        }
    });

    // Check if node already has max number of neighbors.
    if ((isOutgoing && neighbors.length >= constraints.outgoing.range[1])
        || (!isOutgoing && neighbors.length >= constraints.incoming.range[1])) {
        return [];
    }
    ...
    var possibleTypes = [];
    var typesObject = isOutgoing ? constraints.outgoing.types : constraints.incoming.types;
    for (var type in typesObject) {
        if (typesObject.hasOwnProperty(type)) {
            if (type === "*") {
                // Any type is allowed
            ...
            if (index > -1) {
                ...
                var count = 0;
                for (var neighborType in neighborCountByType) {
                    if (neighborCountByType.hasOwnProperty(neighborType)) {
                        if (neighborType in childLeaves) {
                            count += neighborCountByType[neighborType];
                        }
                    }
                }
                ...
                //// Sum up all neighbors whose types match the actualType (=type name starts with actualType string)
                //var count = 0;
                //for (var neighborType in neighborCountByType) {
                //    if (neighborCountByType.hasOwnProperty(neighborType)) {
                //        if (neighborType.type.indexOf(actualType) == 0) {
                //            count += neighborCountByType[neighborType];
                //        }
                //    }
                //}
                //if (count < typesObject[type][1]) {
                //    // Get all the leaf types that match actualType
                //    _.forEach(fv.Config.getLeafNodesTypes(), function(t) {
                //        if (t.type.indexOf(actualType) == 0) {
                ...
                // Leaf type
                if (!(type in neighborCountByType) || neighborCountByType[type] < typesObject[type][1]) {

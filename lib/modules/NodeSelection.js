...
function NodeSelection(flowviz) {
    ...
    this._shadowGreen = Snap('svg').filter(Snap.filter.shadow(0, 0, 10, "green", 1.0));
    this._shadowRed = Snap('svg').filter(Snap.filter.shadow(0, 0, 10, "red", 1.0));
...
NodeSelection.prototype.UpdateSelection = function(node, d3Select) {
    ...
    if(d3Select !== undefined && d3Select !== null) {
        var offset = -(types.length / 2) * 50 + 50;
        console.log(offset);
